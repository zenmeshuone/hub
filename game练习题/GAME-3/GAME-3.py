...
def main(filepath):
	...
	# 加载图片
	newimg = pygame.image.load(GetImagePath(filepath))
	x = 700 / newimg.get_rect().height
	gameImg = pygame.transform.smoothscale(newimg,(int(newimg.get_rect().width * x),
                                                   int(newimg.get_rect().height * x)))
